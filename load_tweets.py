...
def insert_tweet(connection,tweet):
    ...
    with connection.begin() as trans:
        ...
        sql = sqlalchemy.sql.text('''
            ...
            :verified,
            :friends_count, 
            :listed_count,:favourites_count,:statuses_count, :withheld_in_countries) ON CONFLICT (id_users) DO NOTHING
            ''')
        ...
        if tweet.get('in_reply_to_user_id',None) is not None:
            ...
            VALUES(
            :id_users) ON CONFLICT DO NOTHING
            ''')
        ...
        # insert the tweet
            ...
            :text,
            :country_code,
            :state_code,:lang,:place_name,:geo) ON CONFLICT DO NOTHING
            ''')
        ...
        for mention in mentions:
            ...
            res=connection.execute(sql, {
                'id_tweets':tweet['id'],'id_users':mention['id']})
        ...
        for tag in tags:
            sql=sqlalchemy.sql.text('''INSERT INTO tweet_tags (
            id_tweets,
            tag)
            VALUES (
            :id_tweets,
